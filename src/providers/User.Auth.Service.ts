...
setUsername(user): Promise<any> {
  ...
  return this.storage.set('userLoggedin', JSON.stringify(user)).then((res) => {
    // this.setUsername(user);
  ...
  deletePrayers(prayerid): Observable<any> {
    const params = new HttpParams().set('prayerid', prayerid.toString());
    return this.http
      .delete(`${this.baseUrl}/deletePushnetwork`, { params })
      .pipe(
        map(res => {
          console.log(res);
        })
    ...
    updatePrayer(prayer): Observable<any> {
      console.log(prayer);
    return this.http.put(`${this.baseUrl}/updatePrayers`, { data: prayer }).pipe(
      map(res => {
        console.log(res);
      })
    );
  ...
  createPrayer = (prayer) => {
    return this.http.post(`${this.baseUrl}/storePushnetwork`, {data: prayer}).pipe(map((response) => {
      return response;
    }));
  }
}
